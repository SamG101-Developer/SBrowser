        ...
        -> ext::any
{
    return v8pp::class_<node>{isolate}
            ...
            .template function("isEqualNode", &node::is_equal_node)
            .template function("isDefaultNamespace", &node::is_default_namespace)
            .template function("lookupPrefix", &node::lookup_prefix)
            .template function("lookupNamespaceURI", &node::lookup_namespace_uri)
            .template function("compareDocumentPosition", &node::compare_document_position)
            .template function("getRootNode", &node::get_root_node)
            .template function("cloneNode", &node::clone_node)
            .template function("insertBefore", &node::insert_before)
            .template function("appendChild", &node::append_child)
            .template function("replaceChild", &node::replace_child)
            .template function("removeChild", &node::remove_child)
            .template var("nodeName", &node::node_name)
            .template var("nodeValue", &node::node_value)
            .template var("textContent", &node::text_content)
            .template var("baseURI", &node::base_uri)
            .template var("isConnected", &node::is_connected)
            .template var("childNodes", &node::child_nodes)
            .template var("parentNode", &node::parent)
            .template var("parentElement", &node::parent_element)
            .template var("ownerDocument", &node::owner_document)
            .template var("firstChild", &node::first_child)
            .template var("lastChild", &node::last_child)
            .template var("previousSibling", &node::previous_sibling)
            .template var("nextSibling", &node::next_sibling)
            .auto_wrap_objects();
}
