...
dom::nodes::node::normalize() const {
    ...
    for (text* text_node: helpers::trees::descendant_exclusive_text_nodes(this)) {
        ...
        ext::string data = helpers::trees::contiguous_exclusive_text_nodes(text_node)
                .transform<ext::string>([](text* contiguous_text_node) -> ext::string {return contigious_text_node->data;})
                .join("");

        helpers::texts::replace_data(text_node, length, 0, data)
        auto* current_node = node->next_sibling;

        auto* live_ranges = javascript::realms::surrounding_agent()->Global()->Get(javascript::realms::surrounding_agent(), "live_ranges");
        while (helpers::trees::is_exclusive_text_node(current_node)) {
            for (auto* range: live_ranges->filter([current_node](auto* range) {return range->start_container == current_node;})) {
                range->start_offset += length;
                range->start_container = text_node;
            }

            for (auto* range: live_ranges->filter([current_node](auto* range) {return range->end_container == current_node;})) {
                range->end_offset += length;
                range->end_container = text_node;
            }

            for (auto* range: live_ranges->filter([current_node](auto* range) {return range->start_container == current_node->parent_node;})) {
                range->start_offset = length;
                range->start_container = text_node;
            }

            for (auto* range: live_ranges->filter([current_node](auto* range) {return range->end_container == current_node->parent_node;})) {
                range->end_offset = length;
                range->end_container = text_node;
            }
...
bool dom::nodes::node::is_equal_node(node* other) const {return other_node and helpers::node_internals::equals(this, other);}

bool dom::nodes::node::is_default_namespace(ext::cstring& namespace_) const {return namespace_ == helpers::node_internals::locate_namespace(this, "");}
...
dom::nodes::node::lookup_prefix(
    ...
    if (auto* element_node = dynamic_cast<const element*>(this))
        return helpers::node_internals::locate_namespace_prefix(element_node, namespace_);

    if (auto* document_node = dynamic_cast<const document*>(this))
        return helpers::node_internals::locate_namespace_prefix(document_node->document_element);
    ...
    if (auto* attribute_node = dynamic_cast<const attr*>(this))
        return helpers::node_internals::locate_namespace_prefix(attribute_node->owner_element, namespace_);

    return parent_element
            ? helpers::node_internals::locate_namespace_prefix(parent_element, namespace_)
            : nullptr;
}


ext::string dom::nodes::node::lookup_namespace_uri(ext::cstring& prefix) const {return helpers::node_internals::locate_namespace(this, prefix);}

unsigned short
dom::nodes::node::compare_document_position(node* other) const {
    if (this == other) return 0;

    const node* node_1 = other;
    const node* node_2 = this;
    attr* attr_1 = nullptr;
    attr* attr_2 = nullptr;
